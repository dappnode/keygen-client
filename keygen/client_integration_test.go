...
func TestIntegration_FullFlow(t *testing.T) {
	...
	key1, httpCode, err := c.CreateLicense(ctx, policyID, LicenseMetadata{
		SubscriptionID: subscriptionID1,
		CustomerEmail:  customerEmail1,
	})
	if err != nil {
		t.Fatalf("CreateLicense1: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("created license1 key: %s, HTTP code: %d", key1, httpCode)
	...
	key2, httpCode, err := c.CreateLicense(ctx, policyID, LicenseMetadata{
		SubscriptionID: subscriptionID2,
		CustomerEmail:  customerEmail2,
	})
	if err != nil {
		t.Fatalf("CreateLicense2: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("created license2 key: %s, HTTP code: %d", key2, httpCode)
	...
	key3, httpCode, err := c.CreateLicense(ctx, otherPolicyID, LicenseMetadata{
		SubscriptionID: subscriptionID3,
		CustomerEmail:  customerEmail3,
	})
	if err != nil {
		t.Fatalf("CreateLicense3: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("created license3 key: %s, HTTP code: %d", key3, httpCode)

	// --- Resolve license IDs ---
	t.Logf("Resolving license IDs from keys")
	licID1, httpCode, err := c.ResolveLicenseID(ctx, key1)
	if err != nil {
		t.Fatalf("ResolveLicenseID1: %v (HTTP %d)", err, httpCode)
	}
	licID2, httpCode, err := c.ResolveLicenseID(ctx, key2)
	if err != nil {
		t.Fatalf("ResolveLicenseID2: %v (HTTP %d)", err, httpCode)
	}
	licID3, httpCode, err := c.ResolveLicenseID(ctx, key3)
	if err != nil {
		t.Fatalf("ResolveLicenseID3: %v (HTTP %d)", err, httpCode)
	}

	t.Cleanup(func() {
		t.Logf("Cleanup: Deactivating machines and deleting licenses")
	_, _, _ = c.DeactivateMachine(ctx, key1, fingerprint1)
	_, _, _ = c.DeactivateMachine(ctx, key2, fingerprint2)
	_, _, _ = c.DeactivateMachine(ctx, key3, fingerprint3)
	_, _ = c.DeleteLicense(ctx, licID1)
	_, _ = c.DeleteLicense(ctx, licID2)
	_, _ = c.DeleteLicense(ctx, licID3)
	})

	// --- Activate machines for each license ---
	t.Logf("Activating machine1: licenseKey=%s, fingerprint=%s", key1, fingerprint1)
	t.Logf("Activating machine1: licenseKey=%s, fingerprint=%s", key1, fingerprint1)
	httpCode, err = c.ActivateMachine(ctx, key1, fingerprint1, "", "")
	if err != nil {
		t.Fatalf("ActivateMachine1: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("Activating machine2: licenseKey=%s, fingerprint=%s", key2, fingerprint2)
	httpCode, err = c.ActivateMachine(ctx, key2, fingerprint2, "", "")
	if err != nil {
		t.Fatalf("ActivateMachine2: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("Activating machine3: licenseKey=%s, fingerprint=%s", key3, fingerprint3)
	httpCode, err = c.ActivateMachine(ctx, key3, fingerprint3, "", "")
	if err != nil {
		t.Fatalf("ActivateMachine3: %v (HTTP %d)", err, httpCode)
	}

	// --- Validate each license with correct and incorrect fingerprints ---
	t.Logf("Validating license1 with correct fingerprint")
	val, httpCode, err := c.Validate(ctx, key1, fingerprint1)
	if err != nil || !val.Valid {
		t.Fatalf("Validate license1 (correct fp): %v %+v (HTTP %d)", err, val, httpCode)
	}
	t.Logf("Validating license1 with license2's fingerprint (should be invalid)")
	val, httpCode, err = c.Validate(ctx, key1, fingerprint2)
	if err != nil {
		t.Fatalf("Validate license1 (wrong fp): %v (HTTP %d)", err, httpCode)
	}
	if val.Valid {
		t.Fatalf("Validate license1 (wrong fp): expected invalid, got %+v (HTTP %d)", val, httpCode)
	}
	t.Logf("Validating license1 with license3's fingerprint (should be invalid)")
	val, httpCode, err = c.Validate(ctx, key1, fingerprint3)
	if err != nil {
		t.Fatalf("Validate license1 (wrong fp3): %v (HTTP %d)", err, httpCode)
	}
	if val.Valid {
		t.Fatalf("Validate license1 (wrong fp3): expected invalid, got %+v (HTTP %d)", val, httpCode)
	}
	t.Logf("Validating license3 with license1's fingerprint (should be invalid)")
	val, httpCode, err = c.Validate(ctx, key3, fingerprint1)
	if err != nil {
		t.Fatalf("Validate license3 (wrong fp): %v (HTTP %d)", err, httpCode)
	}
	if val.Valid {
		t.Fatalf("Validate license3 (wrong fp): expected invalid, got %+v (HTTP %d)", val, httpCode)
	}
	...
	val, httpCode, err = c.Validate(ctx, key1, randomFP)
	if err != nil {
		t.Fatalf("Validate license1 (random fp): %v (HTTP %d)", err, httpCode)
	}
	if val.Valid {
		t.Fatalf("Validate license1 (random fp): expected invalid, got %+v (HTTP %d)", val, httpCode)
	}

	// --- Additional: Validate a newly created license (not yet activated) ---
	t.Logf("Creating a new license4 (not activating any machine)")
	key4, httpCode, err := c.CreateLicense(ctx, policyID, LicenseMetadata{
		SubscriptionID: "it4-" + randomHex(8),
		CustomerEmail:  strings.Replace(baseEmail, "@", "+notactivated@", 1),
	})
	if err != nil {
		t.Fatalf("CreateLicense4: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("created license4 key: %s, HTTP code: %d", key4, httpCode)
	...
	val, httpCode, err = c.Validate(ctx, key4, notActivatedFP)
	if err != nil {
		t.Fatalf("Validate license4 (not activated): %v (HTTP %d)", err, httpCode)
	}
	if val.Valid {
		t.Fatalf("Validate license4 (not activated): expected invalid, got %+v (HTTP %d)", val, httpCode)
	}
	// Cleanup license4
	licID4, _, _ := c.ResolveLicenseID(ctx, key4)
	_, _ = c.DeleteLicense(ctx, licID4)

	// --- List licenses by policy and check all are present and isolated ---
	t.Logf("Listing licenses by policyID: %s", policyID)
	lics, httpCode, err := c.ListLicensesByPolicy(ctx, policyID)
	if err != nil {
		t.Fatalf("ListLicensesByPolicy: %v (HTTP %d)", err, httpCode)
	}
	fmt.Println(lics)
	...
	lics3, httpCode, err := c.ListLicensesByPolicy(ctx, otherPolicyID)
	if err != nil {
		t.Fatalf("ListLicensesByPolicy (other): %v (HTTP %d)", err, httpCode)
	}
	found3 = false
	for _, l := range lics3 {
		if l.ID == licID3 {
	...
	httpCode, err = c.ActivateMachine(ctx, key2, fingerprint1, "", "")
	if err == nil {
		t.Fatalf("Expected error activating machine1 with license2's key, got none (HTTP %d)", httpCode)
	} else {
		t.Logf("Got expected error: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("Attempting to activate machine3 with license1's key (should fail)")
	httpCode, err = c.ActivateMachine(ctx, key1, fingerprint3, "", "")
	if err == nil {
		t.Fatalf("Expected error activating machine3 with license1's key, got none (HTTP %d)", httpCode)
	} else {
		t.Logf("Got expected error: %v (HTTP %d)", err, httpCode)
	}
	...
	gotID, httpCode, err := c.GetLicenseBySubscriptionID(ctx, subscriptionID1)
	if err != nil {
		t.Fatalf("GetLicenseBySubscriptionID1: %v (HTTP %d)", err, httpCode)
	}
	if gotID != licID1 {
		t.Fatalf("GetLicenseBySubscriptionID1 mismatch: want %s got %s (HTTP %d)", licID1, gotID, httpCode)
	}
	t.Logf("Getting license by subscriptionID2: %s", subscriptionID2)
	gotID, httpCode, err = c.GetLicenseBySubscriptionID(ctx, subscriptionID2)
	if err != nil {
		t.Fatalf("GetLicenseBySubscriptionID2: %v (HTTP %d)", err, httpCode)
	}
	if gotID != licID2 {
		t.Fatalf("GetLicenseBySubscriptionID2 mismatch: want %s got %s (HTTP %d)", licID2, gotID, httpCode)
	}
	t.Logf("Getting license by subscriptionID3: %s", subscriptionID3)
	gotID, httpCode, err = c.GetLicenseBySubscriptionID(ctx, subscriptionID3)
	if err != nil {
		t.Fatalf("GetLicenseBySubscriptionID3: %v (HTTP %d)", err, httpCode)
	}
	if gotID != licID3 {
		t.Fatalf("GetLicenseBySubscriptionID3 mismatch: want %s got %s (HTTP %d)", licID3, gotID, httpCode)
	}

	// --- Edge: List machines for each license, ensure isolation ---
	t.Logf("Listing machines by licenseID1: %s", licID1)
	ms1, httpCode, err := c.ListMachines(ctx, licID1)
	if err != nil {
		t.Fatalf("ListMachines1: %v (HTTP %d)", err, httpCode)
	}
	if len(ms1) != 1 || ms1[0].Fingerprint != fingerprint1 {
		t.Fatalf("ListMachines1: expected 1 machine with fingerprint1, got %+v (HTTP %d)", ms1, httpCode)
	}
	...
	ms2, httpCode, err := c.ListMachines(ctx, licID2)
	if err != nil {
		t.Fatalf("ListMachines2: %v (HTTP %d)", err, httpCode)
	}
	if len(ms2) != 1 || ms2[0].Fingerprint != fingerprint2 {
		t.Fatalf("ListMachines2: expected 1 machine with fingerprint2, got %+v (HTTP %d)", ms2, httpCode)
	}
	for _, m := range ms2 {
	...
	ms3, httpCode, err := c.ListMachines(ctx, licID3)
	if err != nil {
		t.Fatalf("ListMachines3: %v (HTTP %d)", err, httpCode)
	}
	if len(ms3) != 1 || ms3[0].Fingerprint != fingerprint3 {
		t.Fatalf("ListMachines3: expected 1 machine with fingerprint3, got %+v (HTTP %d)", ms3, httpCode)
	}
	for _, m := range ms3 {
	...
	allMachines, httpCode, err := c.ListAllMachines(ctx)
	if err != nil {
		t.Fatalf("ListAllMachines: %v (HTTP %d)", err, httpCode)
	}
	...
	httpCode, err = c.DeleteLicense(ctx, licID1)
	if err != nil {
		t.Fatalf("DeleteLicense1: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("Ensuring license2 is still valid after deleting license1")
	val, httpCode, err = c.Validate(ctx, key2, fingerprint2)
	if err != nil || !val.Valid {
		t.Fatalf("Validate license2 after deleting license1: %v %+v (HTTP %d)", err, val, httpCode)
	}
	t.Logf("Ensuring license3 is still valid after deleting license1")
	val, httpCode, err = c.Validate(ctx, key3, fingerprint3)
	if err != nil || !val.Valid {
		t.Fatalf("Validate license3 after deleting license1: %v %+v (HTTP %d)", err, val, httpCode)
	}

	// --- Ensure license1 is gone ---
	t.Logf("Ensuring deleted license1 cannot be resolved")
	_, httpCode, err = c.ResolveLicenseID(ctx, key1)
	if err == nil {
		t.Fatalf("Expected error resolving deleted license1 (HTTP %d)", httpCode)
	}
	...
	foundAgain, httpCode, err := c.DeactivateMachine(ctx, key1, fingerprint1)
	if err != nil {
		...
		if !strings.Contains(err.Error(), "no license id found") {
			t.Fatalf("DeactivateMachine1 again: %v (HTTP %d)", err, httpCode)
		} else {
			t.Logf("DeactivateMachine1 again: got expected error after license deletion: %v (HTTP %d)", err, httpCode)
		}
	} else if foundAgain {
		t.Fatalf("DeactivateMachine1 again: expected not found (HTTP %d)", httpCode)
	}

	// --- Clean up license2 and license3 ---
	t.Logf("Deleting license2: %s", licID2)
	httpCode, err = c.DeleteLicense(ctx, licID2)
	if err != nil {
		t.Fatalf("DeleteLicense2: %v (HTTP %d)", err, httpCode)
	}
	t.Logf("Deleting license3: %s", licID3)
	httpCode, err = c.DeleteLicense(ctx, licID3)
	if err != nil {
		t.Fatalf("DeleteLicense3: %v (HTTP %d)", err, httpCode)
	}
	...
	defer func() {
		for _, key := range extraKeys {
			licID, _, _ := c.ResolveLicenseID(ctx, key)
			_, _ = c.DeleteLicense(ctx, licID)
		}
	}()
	...
	httpCode, err = c.do(ctx, http.MethodGet, path, nil, &respPage1)
	if err != nil {
		t.Fatalf("Pagination: fetch page 1: %v (HTTP %d)", err, httpCode)
	}
	...
	httpCode, err = c.do(ctx, http.MethodGet, path, nil, &respPage2)
	if err != nil {
		t.Fatalf("Pagination: fetch page 2: %v (HTTP %d)", err, httpCode)
	}
	...
	httpCode, err = c.do(ctx, http.MethodGet, path, nil, &respMachinesPage1)
	if err != nil {
		t.Fatalf("Pagination: fetch machines page 1: %v (HTTP %d)", err, httpCode)
	}
